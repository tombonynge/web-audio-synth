...
function init() {
    ...
    getJSON().then(function (data) {
        ...
        const notesArray = [];
        for (const n in jsonData) {
            notesArray.push(jsonData[n]);
        }
        ...
        octave.addEventListener("change", (e) => {
            let selectedOctave = parseInt(e.target.value);
            if (selectedOctave >= 0 && selectedOctave <= 8) {
                for (let i = 0; i < keysArray.length; i++) {
                    keysArray[i].freq = notesArray[selectedOctave * 12 + i].frequency;
                }
            }
        });
        ...
        function draw() {
            ...
            for (let i = 0; i < freqBuffer; i++) {
                ...
                freqCtx.fillRect(freqX, freqHeight - barHeight / 2, barWidth, barHeight / 2);
...
async function getJSON() {
    let result = await makeRequest("GET", "json");
    return result;
}
